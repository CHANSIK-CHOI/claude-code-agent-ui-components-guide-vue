...
- `pages/guide/tab/index.vue` — 가이드 페이지 (6개 섹션)
...
4. pill + showViewToggle — 격자/목록 토글 인터랙션
5. Navigation only 패턴 — 코드 스니펫 중심
6. Props 테이블 — Tab Props / TabItem 타입 / Events

## 특이사항

- Tab 컴포넌트에서 `TabItem`, `ViewType` 타입을 named import (컴포넌트 파일에서 export)
