...
본 프로젝트는 다음 두 에이전트를 운영한다. 디자인 가이드가 별도로 정리되지 않은 환경 특성상, 디자인 가이드 역할은 planner가 흡수한다.
...
- **Context7 MCP**: 외부 라이브러리(Radix Vue, @vuepic/vue-datepicker 등)의 props/events/slots 정확도가 결정적인 경우에는 컴포넌트 제작·수정 시 반드시 확인한다. Vue 3.4 / Nuxt 3.10 자체 문법 등 학습 데이터로 충분한 내용은 호출하지 않는다.
