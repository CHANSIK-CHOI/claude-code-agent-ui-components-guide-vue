...
### 2. 타입체크 (토글 방식, 기본 ON / opt-out)

| 항목           | 값                                                                                                |
| -------------- | ------------------------------------------------------------------------------------------------- |
| 스크립트       | `.claude/hooks/typecheck.sh`                                                                       |
| hook 타입      | `Stop` (timeout 180s)                                                                              |
| 토글 플래그    | `.claude/hooks/.typecheck-off` — **존재 시에만** 건너뜀. 없으면 항상 검사(기본 ON, opt-out)        |
...
> **기본 ON 근거**: 본 프로젝트가 표방하는 "자동 품질 검증" 파이프라인이 실효성을 가지려면 타입체크가 기본 동작해야 한다. opt-in(기본 OFF) 방식은 플래그가 `.gitignore` 처리돼 clone·검수 환경에서 항상 꺼져 있었고, 그 결과 타입 에러가 누적될 수 있었다. opt-out으로 전환해 기본 강제하고, 필요 시에만 로컬에서 끈다.
...
> **한계 — CI 수준 게이트가 아님**: 본 hook 은 git 변경 파일 단위 가드라 **이미 커밋된 기존 타입 에러는 잡지 못한다**. 즉 "작업 중 새로 만든 코드"를 막는 작업 단위 안전장치이지, 저장소 전체의 타입 무결성을 보장하는 CI 게이트가 아니다. 전체 검증은 `npm run typecheck`(아래)로 수행하며, 진정한 게이트가 필요하면 CI 파이프라인에서 `npm run typecheck` 를 별도로 돌려야 한다.

#### 켜기 / 끄기 (기본 ON)

| 동작 | 방법                                                                                                          |
| ---- | ------------------------------------------------------------------------------------------------------------- |
| 끄기 | "타입체크 꺼줘" → Claude 가 `.claude/hooks/.typecheck-off` 파일 생성 (내용 무관). 또는 사용자가 직접 생성       |
| 켜기 | "타입체크 켜줘" → 그 `.typecheck-off` 파일 삭제 (기본 상태로 복귀). 또는 사용자가 직접 삭제                    |
