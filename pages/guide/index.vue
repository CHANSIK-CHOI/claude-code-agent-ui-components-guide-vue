  ...
  <div class="claudeGuide">
    <header class="claudeGuide__hero">
      ...
      <p class="claudeGuide__heroDesc">
        ...
        경력으로 Vue를 처음 다루는 퍼블리셔가 일관된 마크업·접근성·디자인 토큰
        품질을 유지할 수 있도록, 컴포넌트 한 개를 만들 때 필요한 단계(명세 작성
        → SFC 구현 → 가이드 페이지 → 라이브러리·메커니즘 검수)를 4개의 전문
        에이전트가 역할 분담합니다. 검수에서 이슈가 발견되면 자동 분기
    ...
    <section class="claudeGuide__section">
      <div class="claudeGuide__sectionHead">
        ...
        <p class="claudeGuide__sectionDesc">
          ...
          이미 결정되어 있고, Claude는 그 결정을 코드로 옮깁니다.
        </p>
      </div>
      ...
      <div class="claudeGuide__principleGrid">
        ...
        >
          <span class="claudeGuide__principleNumber">{{
    ...
    <section class="claudeGuide__section">
      <div class="claudeGuide__sectionHead">
        ...
        <p class="claudeGuide__sectionDesc">
          에이전트는 Claude Code의 sub-agent — 특정 역할만 수행하도록 격리된
          실행 단위입니다. 각 에이전트는 자신의 역할 범위 밖 파일은 절대
          수정하지 않으며, 검수 에이전트(qa·reviewer)는 코드 수정 권한 자체가
          없어 권고만 작성합니다(실제 수정은 publisher가 담당).
        </p>
      </div>
    ...
    <section class="claudeGuide__section">
      <div class="claudeGuide__sectionHead">
        ...
        <p class="claudeGuide__sectionDesc">
          신규 컴포넌트는 1~6단계 전체를, 기존 컴포넌트는 검수 단계(4·5)만 단독
          실행할 수 있습니다. 기획이 변경된 경우에는
          <code>/component-revise</code>로 spec 수정부터 재실행합니다. 1단계는
          plan 모드(읽기·계획만 가능)에서 진행되며, 사용자가 명세를 승인한 직후
          <code>Shift+Tab</code>으로 accept 모드(파일 쓰기 가능)로 수동 전환하면
          2단계 이후가 자동 진행됩니다 — Claude Code는 명령 내부에서 mode 전환을
          자동화할 수 없어 이 한 번의 수동 조작이 필요합니다.
        </p>
      </div>
      ...
      <ol class="claudeGuide__flow">
        <li
          ...
          :key="step.title"
          class="claudeGuide__flowStep"
          :data-tone="step.tone"
        >
          <div class="claudeGuide__flowMarker">
            <span class="claudeGuide__flowNumber">{{
              String(idx + 1).padStart(2, "0")
            }}</span>
          </div>
          <div class="claudeGuide__flowBody">
            ...
            <p v-if="step.mode" class="claudeGuide__flowMode">
              {{ step.mode }}
            </p>
          </div>
        </li>
      ...
      <aside class="claudeGuide__loopback">
        ...
        <ul class="claudeGuide__loopbackList">
          ...
          <li>
            <strong>시니어 리뷰 영역</strong> — 항상 publisher만 실행 (spec 영역
            아님)
          </li>
          <li>
            <strong>최대 2회 루프</strong> — 초과 시 사용자 개입 요청으로 종료
          </li>
        </ul>
      </aside>
    </section>
    ...
    <section class="claudeGuide__section">
      <div class="claudeGuide__sectionHead">
        ...
        <p class="claudeGuide__sectionDesc">
          검수 에이전트가 발견한 이슈는 3단계로 분류되어 처리 방식이 달라집니다.
        </p>
      </div>
      ...
      <div class="claudeGuide__severityGrid">
        ...
        >
          <header class="claudeGuide__severityHead">
            ...
            <span class="claudeGuide__severityAction">{{
              severity.action
            }}</span>
          </header>
