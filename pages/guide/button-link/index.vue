<template>
  <div class="buttonLinkGuidePage">
    ...
    <div class="buttonLinkGuidePage__designRef">
      <strong>디자인 variant</strong>(color · shape · size · round) 및 icon slots 시각 예시는 <code>Button</code>과
      동일합니다. <NuxtLink to="/guide/button" class="buttonLinkGuidePage__designRefLink">Button 가이드</NuxtLink>를
      참고해주세요.
    </div>
    ...
    <section class="buttonLinkGuidePage__section">
      ...
      <p class="buttonLinkGuidePage__note">
        <code>&lt;a&gt;</code>에는 네이티브 <code>disabled</code>가 없으므로 <code>aria-disabled="true"</code> +
        <code>tabindex="-1"</code> + <code>pointer-events: none</code>으로 처리합니다. Button의 네이티브
        <code>disabled</code> 속성과 달리, 링크 탐색을 JavaScript 없이 CSS 레벨에서 차단합니다.
      </p>
    </section>
    ...
    <section class="buttonLinkGuidePage__section">
      ...
      <p class="buttonLinkGuidePage__note">
        <code>to</code> prop → 내부 라우팅 (<code>&lt;NuxtLink&gt;</code> 렌더링). <code>href</code> prop → 외부 URL
        (<code>&lt;a&gt;</code> 렌더링). 둘 다 지정 시 <code>to</code>가 우선합니다.
      </p>
      <div class="buttonLinkGuidePage__row">
        <!-- NuxtLink (to) -->
        <div class="buttonLinkGuidePage__item">
          <span class="buttonLinkGuidePage__itemLabel"> to prop — 내부 페이지 이동 </span>
...
&lt;/ButtonLink&gt;</code></pre>
        ...
        <!-- href (외부) -->
        <div class="buttonLinkGuidePage__item">
          <span class="buttonLinkGuidePage__itemLabel"> href prop — 외부 URL 이동 </span>
          <ButtonLink href="https://example.com" shape="line"> example.com 방문 </ButtonLink>
...
&lt;/ButtonLink&gt;</code></pre>
        ...
        <!-- disabled (링크 없음) -->
        <div class="buttonLinkGuidePage__item">
          <span class="buttonLinkGuidePage__itemLabel"> disabled — to/href 있어도 탐색 차단 </span>
          <ButtonLink to="/guide/button" :disabled="true"> 이동 불가 </ButtonLink>
          <pre
            class="buttonLinkGuidePage__code"
          ><code>&lt;!-- disabled 시 NuxtLink 미사용 → &lt;a aria-disabled="true" tabindex="-1"&gt; --&gt;
...
&lt;/ButtonLink&gt;</code></pre>
    ...
    <section class="buttonLinkGuidePage__section">
      ...
      <p class="buttonLinkGuidePage__note">
        <code>target="_blank"</code> 지정 시 <code>rel</code>에 <code>noopener noreferrer</code>가 없으면 자동으로
        추가됩니다. 탭나이핑(tabnabbing) 보안 취약점을 방어합니다.
      </p>
      <div class="buttonLinkGuidePage__row">
        <!-- rel 미지정 → 자동 추가 -->
        <div class="buttonLinkGuidePage__item">
          <span class="buttonLinkGuidePage__itemLabel"> rel 미지정 → noopener noreferrer 자동 추가 </span>
          <ButtonLink href="https://example.com" target="_blank"> 새 탭에서 열기 </ButtonLink>
...
&lt;!-- → &lt;a href="https://example.com" target="_blank" rel="noopener noreferrer"&gt; --&gt;</code></pre>
        ...
        <!-- rel 일부 지정 → 나머지 보완 -->
        <div class="buttonLinkGuidePage__item">
          <span class="buttonLinkGuidePage__itemLabel"> rel 일부 지정 → 누락분만 보완 </span>
          <ButtonLink href="https://example.com" target="_blank" rel="noopener"> 새 탭 (noopener만 지정) </ButtonLink>
...
&lt;!-- → rel="noopener noreferrer" --&gt;</code></pre>
        ...
        <div class="buttonLinkGuidePage__item">
          <span class="buttonLinkGuidePage__itemLabel"> target="_self" (기본값) → rel 변경 없음 </span>
          <ButtonLink href="https://example.com" shape="line"> 같은 탭에서 열기 (기본값) </ButtonLink>
...
&lt;/ButtonLink&gt;</code></pre>
    ...
    <section class="buttonLinkGuidePage__section">
      ...
      <div class="buttonLinkGuidePage__propsTableWrap">
      ...
      <p class="buttonLinkGuidePage__delegationNote">
        ...
        <code>&lt;a&gt;</code> / <code>&lt;NuxtLink&gt;</code> 요소의 모든 네이티브 HTML 속성 (<code>aria-label</code>,
