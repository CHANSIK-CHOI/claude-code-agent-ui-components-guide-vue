  ...
  <div class="buttonGuidePage">
    ...
    <header class="buttonGuidePage__header">
      ...
      <p class="buttonGuidePage__desc">
        ...
        <code>shape</code> × <code>color</code> × <code>size</code> 조합으로 24가지 variant 지원.
      </p>
    </header>
    ...
    <section class="buttonGuidePage__section">
      ...
      <div v-for="shape in shapes" :key="shape" class="buttonGuidePage__group">
        ...
        <div class="buttonGuidePage__row">
          <div v-for="color in getColors(shape)" :key="color" class="buttonGuidePage__item">
            <span class="buttonGuidePage__itemLabel">{{ color }}</span>
            <Button :shape="shape" :color="color">{{ color }}</Button>
            <pre class="buttonGuidePage__code"><code>&lt;Button :shape="{{ shape }}" color="{{ color }}"&gt;
  {{ color }}
&lt;/Button&gt;</code></pre>
          </div>
        </div>
    ...
    <section class="buttonGuidePage__section">
      ...
      <div class="buttonGuidePage__row">
        <div v-for="size in sizes" :key="size" class="buttonGuidePage__item">
          ...
          <pre class="buttonGuidePage__code"><code>&lt;Button size="{{ size }}"&gt;버튼&lt;/Button&gt;</code></pre>
        </div>
      </div>
    </section>
    ...
    <section class="buttonGuidePage__section">
      ...
      <p class="buttonGuidePage__note">shape="text"에서는 round가 무시됩니다.</p>
      <div class="buttonGuidePage__row">
        <div v-for="shape in roundableShapes" :key="shape" class="buttonGuidePage__item">
          <span class="buttonGuidePage__itemLabel">shape="{{ shape }}" + round</span>
          <Button :shape="shape" :round="true">round ({{ shape }})</Button>
          <pre class="buttonGuidePage__code"><code>&lt;Button :shape="{{ shape }}" :round="true"&gt;
  round
&lt;/Button&gt;</code></pre>
        </div>
      </div>
    ...
    <section class="buttonGuidePage__section">
      ...
      <div class="buttonGuidePage__row">
        <div v-for="shape in shapes" :key="shape" class="buttonGuidePage__item">
          <span class="buttonGuidePage__itemLabel">shape="{{ shape }}" + disabled</span>
          <Button :shape="shape" :disabled="true">비활성 ({{ shape }})</Button>
          <pre class="buttonGuidePage__code"><code>&lt;Button :shape="{{ shape }}" :disabled="true"&gt;
  비활성
&lt;/Button&gt;</code></pre>
        </div>
      </div>
        ...
        <div class="buttonGuidePage__item">
          ...
          <Button>
            <template #leading-icon>
              <Icon size="sm"><SmallGreatSvg /></Icon>
            </template>
            확인
          </Button>
...
&lt;/Button&gt;</code></pre>
  ...
  &lt;template #leading-icon&gt;&lt;Icon&gt;&lt;UserSvg /&gt;&lt;/Icon&gt;&lt;/template&gt;
  나만의 컬렉션
  &lt;template #trailing-icon&gt;&lt;Icon size="sm"&gt;&lt;SmallChevronDownSvg /&gt;&lt;/Icon&gt;&lt;/template&gt;
&lt;/Button&gt;</code></pre>
        </div>
      </div>
    </section>
    ...
    <section class="buttonGuidePage__section">
      ...
      <div class="buttonGuidePage__propsTableWrap">
        <table class="buttonGuidePage__propsTable">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td><code>'xs' | 'sm' | 'md-s' | 'md' | 'lg'</code></td>
              <td><code>'md'</code></td>
              <td>버튼 크기. md-s는 팝업 하단/중간 레이아웃 CTA 전용 (h 4.2rem)</td>
            </tr>
            <tr>
      ...
      <p class="buttonGuidePage__delegationNote">
        ...
        <code>&lt;button&gt;</code> 요소의 모든 네이티브 HTML 속성 (<code>aria-label</code>, <code>autofocus</code>,
...
<script setup lang="ts">
  ...
  const sizes = ['lg', 'md', 'md-s', 'sm', 'xs'] as const
