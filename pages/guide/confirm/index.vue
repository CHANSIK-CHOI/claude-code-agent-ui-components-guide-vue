  ...
  <div class="confirmGuidePage">
    <header class="confirmGuidePage__header">
      ...
      <p class="confirmGuidePage__desc">
        ...
        <code>useConfirm().open()</code>으로 <strong>프로그래매틱하게만</strong> 사용합니다.<br />
        <strong>Promise 방식</strong>과 <strong>Callback 방식</strong> 두 가지 모두 지원합니다.
      </p>
    </header>
    ...
    <section class="confirmGuidePage__section">
      ...
      <div class="confirmGuidePage__group">
        <p class="confirmGuidePage__note">
          <code>await open(config)</code>로 결과를 받습니다. ok → <code>true</code>, cancel/ESC/dim →
          <code>false</code>. <code>title</code>은 헤더 영역에 표시되며, <code>message</code>는 선택값입니다. 아래 결과
          영역에서 resolve 값을 확인하세요.
        </p>
        <div class="confirmGuidePage__row">
          <button type="button" class="confirmGuidePage__demoBtn" @click="handlePromise">Promise 방식 열기</button>
        </div>
        <div class="confirmGuidePage__result">
          ...
          <strong
            ...
            "
          >
            {{ lastResult === null ? '(미실행)' : lastResult ? 'true (확인)' : 'false (취소)' }}
          </strong>
        </div>
        <pre class="confirmGuidePage__code"><code>const confirm = useConfirm()
...
if (result) { /* ok */ }
      ...
      <div class="confirmGuidePage__group">
        <p class="confirmGuidePage__groupTitle">message 생략</p>
        <p class="confirmGuidePage__note">
          <code>message</code>를 전달하지 않으면 body 메시지 문단 없이 타이틀과 버튼만 표시됩니다.
        </p>
        <div class="confirmGuidePage__row">
          <button type="button" class="confirmGuidePage__demoBtn" @click="handleTitleOnly">
            message 없는 Confirm 열기
          </button>
        </div>
        <pre class="confirmGuidePage__code"><code>await confirm.open({
  title: '계속 진행할까요?',
})</code></pre>
      </div>
    </section>
    ...
    <section class="confirmGuidePage__section">
      ...
      <div class="confirmGuidePage__group">
        <p class="confirmGuidePage__note">
          <code>onOk</code> prop이 있으면 Callback 방식으로 동작합니다. 반환값은 <code>void</code>입니다.
        </p>
        <div class="confirmGuidePage__row">
          <button type="button" class="confirmGuidePage__demoBtn" @click="handleCallback">Callback 방식 열기</button>
        </div>
        <div class="confirmGuidePage__result">
          <span class="confirmGuidePage__resultLabel">콜백 결과:</span>
          <strong class="confirmGuidePage__resultValue">{{ callbackResult }}</strong>
        </div>
        <pre class="confirmGuidePage__code"><code>confirm.open({
  title: '주문 취소',
  ...
  onCancel: () => { /* cancel 처리 */ },
    ...
    <section class="confirmGuidePage__section">
      <h2 class="confirmGuidePage__sectionTitle">③ cancelColor</h2>
      <div class="confirmGuidePage__group">
        <p class="confirmGuidePage__note">
          <code>cancelColor: 'secondary'</code>를 전달하면 취소 버튼이 시안색(primary 계열) 외형으로 표시됩니다.
          기본값은 <code>'gray'</code>입니다.
        </p>
        <div class="confirmGuidePage__row">
          <button type="button" class="confirmGuidePage__demoBtn" @click="handleCancelColor">
            cancelColor='secondary' Confirm 열기
          </button>
        </div>
        <pre class="confirmGuidePage__code"><code>confirm.open({
  title: '구독 취소',
  message: '정기 구독을 취소하시겠습니까?',
  okLabel: '구독 유지',
  cancelLabel: '취소',
  cancelColor: 'secondary',
})</code></pre>
    ...
    <!-- ④ okColor -->
    <section class="confirmGuidePage__section">
      <h2 class="confirmGuidePage__sectionTitle">④ okColor</h2>
      <div class="confirmGuidePage__group">
        <p class="confirmGuidePage__note">
        ...
        <div class="confirmGuidePage__row">
          <button type="button" class="confirmGuidePage__demoBtn" @click="handleOkColor">
            okColor='black' Confirm 열기
          </button>
        </div>
        <pre class="confirmGuidePage__code"><code>confirm.open({
  title: '삭제 확인',
  ...
  okColor: 'black',
    ...
    <section class="confirmGuidePage__section">
      ...
      <div class="confirmGuidePage__group">
        <div class="confirmGuidePage__row">
          <button type="button" class="confirmGuidePage__demoBtn" @click="handleCustomLabel">라벨 변경 열기</button>
        </div>
        <pre class="confirmGuidePage__code"><code>confirm.open({
  title: '이용약관 동의',
  ...
  cancelLabel: '거부',
    ...
    <section class="confirmGuidePage__section">
      ...
      <h3 class="confirmGuidePage__tableTitle">공통 config</h3>
      <p class="confirmGuidePage__commonNote">아래 항목은 Alert / Confirm에서 공통으로 지원하는 config입니다.</p>
      <div class="confirmGuidePage__propsTableWrap">
      ...
      <h3 class="confirmGuidePage__tableTitle">Confirm 전용 config</h3>
      <div class="confirmGuidePage__propsTableWrap">
        <table class="confirmGuidePage__propsTable">
          <thead>
            <tr>
          ...
          <tbody>
            <tr>
              ...
              <td><code>string</code></td>
              <td>— (선택)</td>
              <td>body 확인 메시지 텍스트. 없으면 메시지 문단을 렌더링하지 않음</td>
            </tr>
            <tr>
              <td><code>onOk</code></td>
              <td><code>() =&gt; void</code></td>
              <td>—</td>
              <td>Callback 방식 전용. 있으면 Callback 방식, 없으면 Promise 방식</td>
            </tr>
            <tr>
        ...
        <table class="confirmGuidePage__propsTable">
          ...
          <tbody>
            <tr>
              <td>Callback</td>
              <td>
                <code>open(config: ConfirmConfig &amp; &#123; onOk: () =&gt; void &#125;): void</code>
              </td>
              <td><code>void</code></td>
            </tr>
            <tr>
      ...
      <p class="confirmGuidePage__delegationNote">
        <strong>사용 방법</strong>: <code>&lt;Confirm /&gt;</code>를 템플릿에 직접 마크업하지 않습니다.
        <code>useConfirm().open(config)</code>만 호출하면 <code>PopupRenderer</code>가 자동으로 렌더링합니다.
      </p>
...
<script setup lang="ts">
  ...
  async function handleCancelColor() {
    await confirm.open({
      title: '구독 취소',
      message: '정기 구독을 취소하시겠습니까?',
      okLabel: '구독 유지',
