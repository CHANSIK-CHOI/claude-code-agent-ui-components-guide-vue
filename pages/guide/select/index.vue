  ...
  <div class="selectGuidePage">
    ...
    <header class="selectGuidePage__header">
      ...
      <p class="selectGuidePage__desc">
        ...
        라벨 · 도움말 · 에러 텍스트가 필요한 경우
        <code>FormField</code> (molecules)와 조합해 사용합니다.<br />
    ...
    <section class="selectGuidePage__section">
      ...
      <div class="selectGuidePage__group">
        ...
        <p class="selectGuidePage__note">
          라벨이 외부에서 제공되거나 불필요한 경우. <code>aria-label</code>을 직접 전달하세요.<br />
          Trigger 텍스트(placeholder / 선택값)가 1줄을 초과하면 자동으로 말줄임(<code>…</code>) 처리됩니다.
          텍스트 영역이 <code>flex: 1; min-width: 0</code>으로 수축하여 chevron 아이콘 영역을 침범하지 않습니다.
        </p>
        <div class="selectGuidePage__row">
          <div class="selectGuidePage__item selectGuidePage__item--narrow">
            <Select
              ...
              placeholder="카테고리를 선택해주세요카테고리를 선택해주세요카테고리를 선택해주세요카테고리를 선택해주세요카테고리를 선택해주세요카테고리를 선택해주세요카테고리를 선택해주세요카테고리를 선택해주세요카테고리를 선택해주세요카테고리를 선택해주세요카테고리를 선택해주세요카테고리를 선택해주세요"
              aria-label="상품 카테고리"
            />
...
/&gt;</code></pre>
      ...
      <div class="selectGuidePage__group">
        ...
        <p class="selectGuidePage__note">
          <code>FormField</code>의 <code>input-id</code>와 <code>Select</code>의 <code>id</code>를 반드시 동일하게
          설정해야 스크린 리더가 라벨과 컨트롤을 올바르게 연결합니다.
        </p>
        <div class="selectGuidePage__row">
          <div class="selectGuidePage__item selectGuidePage__item--narrow">
...
&lt;/FormField&gt;</code></pre>
      ...
      <div class="selectGuidePage__group">
        ...
        <p class="selectGuidePage__note">
          <code>aria-describedby</code>는 FormField가 생성하는 도움말 요소의 id와 연결합니다. 패턴은
...
&lt;/FormField&gt;</code></pre>
    ...
    <section class="selectGuidePage__section">
      ...
      <div class="selectGuidePage__group">
        ...
        <p class="selectGuidePage__note">
          <code>v-model</code>에 초기값이 설정되면 Trigger에 선택된 옵션의 라벨이 표시됩니다. placeholder는 숨겨집니다.
        </p>
        <div class="selectGuidePage__row">
          <div class="selectGuidePage__item selectGuidePage__item--narrow">
...
&lt;/FormField&gt;</code></pre>
      ...
      <div class="selectGuidePage__group">
        ...
        <p class="selectGuidePage__note">
          <code>options</code> 배열의 개별 항목에 <code>disabled: true</code>를 설정하면 해당 아이템만 선택 불가 상태가
          됩니다. 드롭다운은 정상적으로 열립니다.
        </p>
        <div class="selectGuidePage__row">
          <div class="selectGuidePage__item selectGuidePage__item--narrow">
...
/&gt;</code></pre>
    ...
    <section class="selectGuidePage__section">
      ...
      <div class="selectGuidePage__group">
        ...
        <p class="selectGuidePage__note">
          <code>:error="true"</code>를 전달하면 Trigger 테두리 색상이 <code>$color-danger(#ff5146)</code>로 변경되고
          <code>aria-invalid="true"</code>가 자동 적용됩니다.<br />
        ...
        <div class="selectGuidePage__row">
          <div class="selectGuidePage__item selectGuidePage__item--narrow">
            <FormField label-text="상품 카테고리" input-id="demo-error" error-text="카테고리를 선택해주세요.">
              <Select
                id="demo-error"
                v-model="demoError"
                :options="categoryOptions"
    ...
    <section class="selectGuidePage__section">
      ...
      <div class="selectGuidePage__group">
        ...
        <p class="selectGuidePage__note">
          ...
          아이템이 이를 초과하면 내부 스크롤이 활성화되고, 패널 상하단에 스크롤 버튼(<code>SelectScrollUpButton</code> /
    ...
    <section class="selectGuidePage__section">
      ...
      <div class="selectGuidePage__group">
        ...
        <p class="selectGuidePage__note">
          ...
          Radix Vue가 숨겨진 <code>&lt;input type="hidden"&gt;</code>을 내부 렌더링하여 form submit 시 선택값이
          전송됩니다.
        </p>
        <div class="selectGuidePage__row">
          <div class="selectGuidePage__item selectGuidePage__item--narrow">
...
/&gt;</code></pre>
      ...
      <div class="selectGuidePage__group">
        ...
        <p class="selectGuidePage__note">
          <code>aria-*</code>, <code>class</code>, <code>style</code>, <code>tabindex</code> 등 나머지 속성은
          <code>SelectTrigger</code>에 위임됩니다.<br />
...
/&gt;</code></pre>
    ...
    <section class="selectGuidePage__section">
      ...
      <p class="selectGuidePage__note">
        <code>variant="filter"</code>는 상품 목록 등 인라인 필터 UI에서 사용하는 compact 형태입니다.<br />
        trigger 너비가 내용에 맞게 줄어들며, 높이는 패딩 기반으로 결정됩니다. Figma 노드: <code>40004271:6839</code>
      </p>

      <div class="selectGuidePage__group">
        <p class="selectGuidePage__groupTitle">기본 (default 상태)</p>
        <p class="selectGuidePage__note">
          인라인 필터 컨트롤로, trigger 너비가 내용에 맞게 줄어드는 compact 형태입니다.<br />
          드롭다운 너비는 trigger 너비를 최솟값으로 하되 화면 경계를 넘지 않으며,
          긴 텍스트는 말줄임(<code>…</code>) 처리됩니다.
        </p>
        <div class="selectGuidePage__row">
...
/&gt;</code></pre>
      ...
      <div class="selectGuidePage__group">
        ...
        <p class="selectGuidePage__note">
          실제 상품 목록 필터 UI처럼 여러 filter Select를 가로로 나란히 배치한 예시입니다.
        </p>
        <div class="selectGuidePage__row">
          <div class="selectGuidePage__item">
            ...
            <pre class="selectGuidePage__code"><code>&lt;div class="filterRow"&gt;
...
&lt;/div&gt;</code></pre>
    ...
    <section class="selectGuidePage__section">
      ...
      <div class="selectGuidePage__group">
        ...
        <p class="selectGuidePage__note">
          <code>name</code>, <code>required</code>는 <strong>SelectRoot</strong>로, <code>aria-describedby</code>는
          <strong>SelectTrigger(button)</strong>으로 자동 분류됩니다. 사용하는 쪽에서 구분 없이 한 줄에 전달하면
          내부에서 자동으로 올바른 위치에 배분됩니다.
        </p>
        <div class="selectGuidePage__row">
          <div class="selectGuidePage__item selectGuidePage__item--narrow">
...
/&gt;</code></pre>
      ...
      <div class="selectGuidePage__propsTableWrap">
        <table class="selectGuidePage__propsTable">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td><code>false</code></td>
              <td>
                에러 상태. Trigger 테두리 색상을 <code>$color-danger(#ff5146)</code>로 변경 +
                <code>aria-invalid="true"</code> 자동 적용. 에러 메시지 텍스트는 FormField 담당
              </td>
            </tr>
            <tr>
              <td><code>variant</code></td>
              <td><code>'default' | 'filter'</code></td>
              <td><code>'default'</code></td>
              <td>
                <code>default</code> — 폼용 (높이 46px 고정, 너비 100%).<br />
                <code>filter</code> — 인라인 필터용 compact (padding 기반 높이, fit-content 너비)
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div class="selectGuidePage__delegationNote">
        ...
        <p>
          선언된 Props 외 속성은 <code>useAttrs()</code>로 분리되어 두 그룹으로 자동 배분됩니다. React의
          <code>{...rest}</code> spreading과 동일한 동작이지만, Select는 Root / Trigger 두 위치로 나뉩니다.
        </p>
        ...
        <table class="selectGuidePage__delegationTable">
          ...
          <tbody>
            <tr>
              ...
              <td>폼 제출 시 필드명. Radix Vue가 hidden input을 자동 생성해 전송</td>
            </tr>
            <tr>
            ...
            <tr>
              <td><code>dir</code></td>
              <td>텍스트 방향 (<code>ltr</code> / <code>rtl</code>)</td>
            </tr>
            <tr>
              <td><code>open</code></td>
              <td>드롭다운 열림 상태 제어 (<code>v-model:open</code>)</td>
            </tr>
            <tr>
              <td><code>defaultOpen</code></td>
              <td>초기 열림 상태 (비제어 모드)</td>
            </tr>
            <tr>
              <td><code>defaultValue</code></td>
              <td>초기 선택값 (비제어 모드, v-model 없이 사용 시)</td>
            </tr>
            <tr>
...
/&gt;</code></pre>
        <p>
          ※ <code>id</code>는 양쪽 제외 — 내부 <code>selectId</code>가 단독 처리 (<code>&lt;label for&gt;</code> 연동)
        </p>
      </div>
      ...
      <p class="selectGuidePage__radixNote">
        ...
        사용할 수 있습니다. 전체 API는
        <a href="https://www.radix-vue.com/components/select.html" target="_blank" rel="noopener noreferrer"
...
<script setup lang="ts">
  ...
  const categoryOptions = [
    {
      value: 'top',
      label: '상의상의상의상의상의상의상의상의상의상의상의상의상의상의상의상의상의상의상의상의상의상의상의상의상의상의',
    },
  ...
  // filter variant 케이스용 — 긴 텍스트 포함 (드롭다운 max-width + 말줄임 동작 확인)
  const filterCategoryOptions = [
    ...
    { value: 'dress', label: '원피스' },
    { value: 'shoes', label: '신발' },
    { value: 'long', label: '매우 긴 옵션 텍스트 — 오버플로우 테스트용 항목입니다' },
  ];
  ...
  // filter variant 데모 상태값
  const demoFilter = ref<string | undefined>(undefined);
  const demoFilterDisabled = ref<string | undefined>(undefined);
  const demoFilterError = ref<string | undefined>(undefined);
  const demoFilterMulti1 = ref<string | undefined>(undefined);
  const demoFilterMulti2 = ref<string | undefined>(undefined);
