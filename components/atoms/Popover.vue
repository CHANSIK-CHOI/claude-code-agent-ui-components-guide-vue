<template>
  <PopoverRoot :open="proxyOpen" @update:open="handleOpenChange">
    <PopoverTrigger v-bind="triggerAttrs" as-child :aria-controls="panelId" :aria-expanded="proxyOpen">
      <slot name="trigger" />
    </PopoverTrigger>
    ...
    >
      <div ref="panelInnerRef" class="popover__inner">
        <div v-if="title" class="popover__header">
          ...
          <button type="button" class="popover__close" aria-label="닫기" @click="handleClose">
...
<script setup lang="ts">
  ...
  // React의 useId() 대체 패턴 (Vue 3.4는 useId 미지원)
  let _popoverCount = 0;
  ...
  function startWrapperFix() {
    ...
    const wrapper = panelInnerRef.value?.closest('[data-radix-popper-content-wrapper]') as HTMLElement | null;
    if (!wrapper) return;
    ...
    _wrapperObserver.observe(wrapper, { attributes: true, attributeFilter: ['style'] });
  }
  ...
  const proxyOpen = computed(() => (props.open !== undefined ? props.open : internalOpen.value));
...
<style lang="scss" scoped>
  ...
  :deep(.#{$b}__panel) {
    ...
    width: calc(100% - ($screen-padding-x * 2)) !important; // wrapper 너비 채움 (= min(600px, viewport-scrollbar))
  ...
  :deep(.#{$b}__panel[data-state='closed'] .#{$b}__inner:not([data-has-been-opened])) {
    opacity: 0;
  }
