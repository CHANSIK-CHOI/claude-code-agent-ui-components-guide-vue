<template>
  <SelectRoot v-bind="rootAttrs" :disabled="disabled" v-model="proxyValue" @update:open="emit('open-change', $event)">
    <SelectTrigger
      v-bind="triggerAttrs"
      :id="selectId"
    ...
    >
      ...
      <span v-if="!proxyValue" class="select__placeholder">{{ placeholder }}</span>
    ...
    <SelectPortal>
      ...
      >
        ...
        <SelectScrollDownButton class="select__scrollBtn select__scrollBtn--down">
...
<script setup lang="ts">
  ...
  const SELECT_ROOT_PROPS = ['defaultValue', 'open', 'defaultOpen', 'required', 'name', 'autocomplete', 'dir'] as const;

  const attrs = useAttrs();

  const _internalId = ref(`select-${++_selectUid}`);
  const selectId = computed(() => (attrs.id as string | undefined) ?? _internalId.value);

  const rootAttrs = computed(() =>
    Object.fromEntries(Object.entries(attrs).filter(([k]) => (SELECT_ROOT_PROPS as readonly string[]).includes(k)))
  );

  const triggerAttrs = computed(() =>
    Object.fromEntries(
      Object.entries(attrs).filter(([k]) => !(SELECT_ROOT_PROPS as readonly string[]).includes(k) && k !== 'id')
    )
  );
...
<style lang="scss" scoped>
  ...
  :deep(.#{$b}__content--filter) {
    ...
    min-width: var(--radix-select-trigger-width); // Trigger 너비 최소 보장 (position="popper" 시 자동 주입)
    max-width: var(--radix-select-content-available-width); // 화면 가용 너비 초과 방지
  }
