<template>
  ...
  <template v-if="alwaysOpen">
    <div class="tooltip__alwaysOpen">
      <div v-bind="triggerAttrs" :aria-describedby="tooltipId" class="tooltip__trigger">
        <slot name="trigger" />
      </div>
      <div
        :id="tooltipId"
        class="tooltip__bubble"
        :class="[`tooltip__bubble--${color}`, `tooltip__bubble--arrow-${arrowAlign}`, `tooltip__bubble--side-${side}`]"
  ...
  <TooltipProvider v-else :delay-duration="0">
    <TooltipRoot v-bind="rootAttrs" :open="isOpen" :delay-duration="delayDuration" @update:open="handleOpenChange">
      ...
      <TooltipPortal :disabled="!portal" :to="container">
        <TooltipContent
          ...
          :class="[`tooltip__content--${color}`, `tooltip__content--arrow-${arrowAlign}`]"
          :style="customStyle"
          :side="side"
          :side-offset="offsetY"
          :align="radixAlign"
...
<script setup lang="ts">
  ...
  const rootAttrs = computed(() =>
    Object.fromEntries(Object.entries(attrs).filter(([k]) => (TOOLTIP_ROOT_PROPS as readonly string[]).includes(k)))
  );
  ...
  const arrowFill = computed(() => props.bgColor ?? (props.color === 'primary' ? '#00addb' : '#333333'));
  ...
  const textColorStyle = computed(() => (props.textColor ? { color: props.textColor } : {}));

  // ── [분기 B] alwaysOpen=true 전용 ────────────────────────────────────

  // tooltipId — Math.random 기반 고유값 (Vue 3.4이므로 useId() 사용 불가)
  ...
  const bubbleStyle = computed(() => {
    const vertical =
      props.side === 'top' ? { bottom: `calc(100% + ${props.offsetY}px)` } : { top: `calc(100% + ${props.offsetY}px)` };
