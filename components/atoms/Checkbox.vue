...
const checkboxId = computed(
  () => (props.id ?? (attrs.id as string | undefined)) ?? `checkbox-${_uid}`,
)

// $attrs에서 id 제외한 나머지를 CheckboxRoot에 위임
const rootAttrs = computed(() =>
  Object.fromEntries(
    Object.entries(attrs).filter(([k]) => k !== 'id'),
  ),
)
