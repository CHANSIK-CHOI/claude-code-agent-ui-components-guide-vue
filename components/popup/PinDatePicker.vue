<template>
  <BottomSheet
    ...
    @close="handleClose"
  >
    <div ref="bodyRef" class="pinDatePicker__body">
      <van-date-picker
...
<script setup lang="ts">
  import type { PickerOption, PickerConfirmEventParams, PickerChangeEventParams, DatePickerColumnType } from 'vant';
  ...
  const props = withDefaults(
    defineProps<{
      ...
      filter?: (type: string, options: PickerOption[], values: string[]) => PickerOption[];
    }>(),
    {
      modelValue: () => [],
      open: false,
      ...
      formatter: (type: string, option: PickerOption): PickerOption => ({
        ...option,
        text: `${option.text}${({ year: '년', month: '월', day: '일' } as Record<string, string>)[type] ?? ''}`,
      }),
    }
  );
