...
<script lang="ts">
  ...
  export const AccordionItem = {
    ...
    setup(props: { value: string; disabled?: boolean }, { slots, attrs }: SetupContext) {
      return () =>
        h(
          RadixAccordionItem,
    ...
    setup(props: { headTrigger?: boolean }, { slots, attrs }: SetupContext) {
      return () => {
        ...
        const defaultContent = props.headTrigger
          ...
          : hasSlot
            ? slots.default?.()
            : h(Icon, { size: 'sm' }, { default: () => h(SmallChevronDownSvg, { 'aria-hidden': 'true' }) });

        return h(
          RadixAccordionTrigger,
          {
            ...attrs,
            class: props.headTrigger ? 'accordion__trigger--head' : 'accordion__trigger--icon',
          },
    ...
    setup(props: { contentAnimation?: boolean }, { slots }: SetupContext) {
      return () =>
        h(
          ...
          { class: props.contentAnimation ? 'accordion__content--animated' : undefined },
          { default: () => slots.default?.() }
        );
    },
  };
  ...
  const rootAttrs = computed(() => {
    const filtered = Object.fromEntries(
      Object.entries(attrs).filter(
        ([k]) =>
          (ACCORDION_ROOT_PROPS as readonly string[]).includes(k) || (ROOT_EVENT_PROPS as readonly string[]).includes(k)
      )
    );
